#!/usr/bin/env python3
"""Generate the bundled Grafana dashboard for the LeSysBot monitoring stack.
...
It writes:
  * system-overview.json  — node_exporter (CPU/mem/disk/net/temps) + NVIDIA GPU
...
def temps_section_node(g: Layout) -> None:
    """CPU / disk / sensor temperatures (hwmon + thermal zones)."""
    ...
    g.ts("Other Sensors — ACPI zone / chipset / Wifi radio",
         ...
         desc="ACPI thermal zones: CPU package (x86_pkg_temp), chassis (acpitz), "
              "Wifi radio (iwlwifi), etc.")




def gpu_section(g: Layout) -> None:
    """GPU panels — populated when nvidia_gpu_exporter is running."""
...
def build_node() -> dict:
    ...
    g.ts("Memory Usage",
         ...
         "bytes", minv=0)
    g.ts("Swap Usage",
         [(f'node_memory_SwapTotal_bytes{inst} - node_memory_SwapFree_bytes{inst}', "used"),
          (f'node_memory_SwapFree_bytes{inst}', "free")], "bytes", minv=0)
