...
def write_package_module(repo_root: Path) -> Path | None:
    ...
    so an installed copy can't read it. Same fix the web UI uses in
    `management/page.py`: inline the asset as Python. Only the grids go in —
    `core/banner.py` renders `MARK` (16px) through Rich, and `management/page.py`
