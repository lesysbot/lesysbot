...
Two rules hold throughout: **nothing here installs anything** (only pip deps and LeSysBot's own packages are auto-fixed, elsewhere), and **nothing here runs sudo** — where the only real fix needs root, the fix is *text*. `_binary_fix` deliberately refuses to guess `apt install <command>`: package names routinely differ from the command they install, and `apt install nvidia-smi` is not a thing. Exact commands only where known correct; otherwise name the binary and let the user pick.

**GPU is the case this layer exists for** (`prereq/gpu.py`). Generalizes what `start.sh` already does: **key on the reading tool, never the card**, because `nvidia_gpu_exporter` shells out to `nvidia-smi` and a driverless GPU is unscrapeable. `lspci` is consulted *only to explain* — so a box with an RTX 3080 and no driver reads as "present, driver not installed → …" rather than "no GPU".
...
**Every entry now points into this repo** (`lesysbot/lesysbot/tools`, `lesysbot/lesysbot/dashboards/<name>`). They used to point at a companion `lesysbot-packages-official` repo; the Linux-only refactor folded its tools into `tools/` and its two dashboards into `dashboards/`, leaving it a duplicate of what already ships, so it was retired and its listings were re-pointed rather than deleted — `lesysbot search --kind dashboard` returning nothing reads as "dashboards aren't installable". Nothing tied a listing to anything real, which is why a dead source stayed valid-looking; `tests/test_catalog.py::test_every_bundled_entry_points_into_this_repo` now walks the tree for each entry, so renaming a package breaks the build rather than a user's install. Since those entries are all bundled and seeded anyway, installing one is an *update* — it pulls the repo's current copy, which is ahead of the wheel's between releases. **`catalog.json` has a published twin** at `https://lesysbot.github.io/catalog.json` (the docs site's `content/static/catalog.json`) that `lesysbot search --refresh` prefers over the bundled copy; change one and change both, or a refresh silently reinstates the old sources.
...
`dashboards/network-traffic/` and `dashboards/gpu-detail/` came home from the retired `lesysbot-packages-official` repo and are bundled and seeded like System Overview. Neither costs a user anything on a machine that can't feed it: `gpu-detail` declares `gpu: nvidia` plus its exporter metric and `network-traffic` declares `service: prometheus`, so the withholding rule keeps an unusable one un-provisioned rather than showing blank panels. `network-traffic` lost its windows_exporter branch in the port — node_exporter's metric names are the only ones left, and a test asserts the hybrid didn't survive.
...
`tool.py`, `@tool` vs `CLITool`, `requires` gating, package-local
...
prints the two lines and carries on. `--with-ollama` overrides.

## Documentation structure

Docs are **short and user-first**. Each page opens with the one command or answer
that works, uses short plain sentences, and links to the page that owns a fact
instead of repeating it. Internals belong in `docs/architecture.md`, not in the
user pages; a `<details><summary><b>…</b></summary>` block is for *optional
how-to* (another install method, a by-hand setup), not for explaining internals.
Keep examples copy-pasteable and verified against the real CLI. Only
`architecture.md` and `CONTRIBUTING.md` number their sections.

All guides live in `docs/` (the root keeps only `README.md`, `CONTRIBUTING.md`, and this file). `docs/README.md` is the index, grouped **Start here → Use it → Extend it → Run it → For contributors**. Ownership: install/uninstall → `getting-started.md`; chatting, `/` commands, tool on/off → `usage.md`; Telegram/Discord setup → `adapters.md`; Grafana use, snapshot sharing, installing/writing dashboards → `dashboards.md` (`dashboard/README.md` is the stack's technical reference); installing tools → `installing-tools.md`; writing, sharing, and the Claude Code plugin → `writing-tools.md`; the service → `service.md`; trust boundaries → `security.md`; every symptom→fix → `troubleshooting.md`; internals, including how to add a chat platform → `architecture.md`. When changing behaviour, update the guide that documents it; when adding a page, slot it into `docs/README.md`, the root README's "What next?" table if it's a common task, **and** the docs site (`content/<version>/nav.json` + the `GUIDES` map in `scripts/import-docs.js`). The site imports every guide from here except its own `overview.md`; a removed or renamed slug needs an entry in the site's `content/<version>/redirects.json` so old links keep working.

`skills/` holds **agent-facing skills** (Claude Skill format, one `SKILL.md` per job) distilled from the docs so an AI agent can operate/extend LeSysBot without reading `docs/` or source — deliberately copyable as a unit, so unlike the docs it *duplicates* facts. The mapping mirrors the docs (`docs/configuration.md` ↔ `skills/configure-lesysbot/`, `docs/service.md` ↔ `skills/manage-service/`, `docs/dashboards.md` ↔ `skills/manage-dashboards/`, …; `skills/README.md` is the routing index). When a behaviour change updates a doc page, update the matching skill too. (`.claude/skills/` is separate: project skills for working *on this repo* in Claude Code.)

This repo is also a **Claude Code plugin marketplace** (`.claude-plugin/marketplace.json` + the `lesysbot-tool-dev` plugin in `claude-plugin/`): it distributes a repo-agnostic `add-tool` skill to tool authors in *other* repos — a tool repo commits a `.claude/settings.json` (`extraKnownMarketplaces`/`enabledPlugins`) so cloners get prompted to install it (docs: `docs/writing-tools.md` → "Let Claude Code write it"). The plugin is deliberately unversioned (every push is a new version). When tool-package conventions change, update **both** `.claude/skills/add-tool/` (core-repo variant) and `claude-plugin/lesysbot-tool-dev/skills/add-tool/` (generalized variant — no relative doc links, no `tools/`-dir or catalog/tests assumptions).
