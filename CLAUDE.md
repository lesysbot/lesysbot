...
pip install -e ".[dev]"
# The `lesysbot` console script imports whatever `lesysbot` is installed: a stale
# NON-editable build in site-packages shadows this repo, so code/tool edits appear
# to do nothing. Check with `python -c "import lesysbot; print(lesysbot.__file__)"`
# (must point here) and fix with `pip install -e .`.

# Run the bot (CLI mode — no messaging credentials needed)
lesysbot chat --model qwen3.5:4b
lesysbot chat -v                               # verbose logging
lesysbot -c ~/.lesysbot/config.yaml            # explicit config
...
lesysbot dashboard current | render | reset | start | stop
...
**Argument coercion:** `call()` runs `coerce_arguments(properties, arguments)` before invoking a tool. Arguments arrive as *text* from two directions that can't type them — a `/tool size_mb=10` slash command parsed out of a chat message, and an LLM quoting numbers in its tool-call JSON. Only `str` values are converted, and only where the declared JSON type says to, so correctly-typed input passes through untouched; `array`/`object` are left alone (no sane single-field text form). An uncoercible value returns `Tool error: <param> must be a whole number, got 'abc'` instead of a traceback from inside the tool. This is what makes the native-slash-command round trip through text lossless.
...
**Native slash commands (`lesysbot/messaging/commands.py`):** typing `/disk_usage path=/` always reaches `Agent._handle_slash` on every adapter, so calling a tool without the LLM was never the missing part — **discoverability** was. Telegram's `/` menu and Discord's command picker only offer commands a bot has *registered with the platform*. `commands.py` builds one list of `CommandSpec`s from the registry and each adapter renders it in its own idiom: Telegram a flat name+description menu (`bot.set_my_commands`, no typed parameters — arguments stay free text), Discord one **typed application command per tool** (`DiscordAdapter._build_commands`). Keeping the specs in one module is what stops the two platforms drifting on which tools are offered.

Command names must be `[a-z0-9_]{1,32}` (the intersection of both platforms' rules) and a bot may register 100; descriptions are clamped to Discord's 100 characters, the tighter limit, so one spec is valid on both. A tool whose name doesn't fit is **skipped with a warning, never renamed** — a renamed command would no longer match the tool name `_handle_slash`, `/help` and the docs all use. Disabled and platform-unavailable tools are left out: a command whose only possible reply is "that's disabled" teaches nothing and costs one of the 100 slots.
...
**Optional dependencies:** only the CLI provider, the registry and the `tools`/`setup` subcommands are in the base install. Chat platforms are extras — `telegram` (`python-telegram-bot`), `discord` (`discord.py`, floored at **>=2.4** because that is the first release to depend on `audioop-lts` on Python 3.13, where `audioop` left the stdlib — CI's matrix includes 3.13, so an older pin would fail to import there), and `all` (both; `dev` pulls `all`). `scripts/install.{sh,ps1}` install `.[all]` so every option the wizard offers works. `_run()` wraps the lazy adapter imports in one `try/except ImportError` that names the missing extra instead of dumping a traceback — for a background service that message is the user's only clue. CI's `base-install` job installs bare `.`, asserts `telegram`/`discord`/`aiohttp` are absent (`aiohttp` still counts — discord.py pulls it, so the assertion stays meaningful), and smoke-tests the CLI, so the base install can't silently regain a heavy dep.
...
**A LeSysBot install has exactly ONE dashboard.** Installing a dashboard *replaces* the current one — `installer._replace_current_dashboard()` drops the previous package's directory and lock entry — so there is one Grafana page at one fixed uid rather than a set the user must choose between. `ArtifactLock.current_dashboard()` is the single accessor; more than one entry is an invariant violation reported by `lesysbot doctor` (`cli/doctor.py:_dashboard_invariant`), not a state to support. A repo offering several dashboards has them **deferred** (`_select_one_dashboard`) and listed by name, requiring `--only NAME`: silently taking one is precisely the `_pick_uid` bug this replaced. Its *tools* still install in the same command — `lesysbot install official` is the documented first command and the official collection carries seven dashboards, so failing the whole install over an ambiguity in one kind would be the worse answer. A repo with *nothing but* several dashboards has no unambiguous part left, so that case raises.

`render.py` turns the installed dashboard package into the JSON Grafana provisions, written to `dashboard/grafana/dashboards/generated/` — the **directory** all three launch paths mount (both composes and `install-macos.sh`). It writes exactly one file, `OUTPUT_NAME` (`lesysbot.json`), and `_sweep()` unlinks every other `*.json` there on each render: that self-corrects both leftovers from the per-package era and the standalone stack's own output, either of which would show in Grafana as a second page nobody chose. `render_one` stamps `model["uid"] = DASHBOARD_UID` (`"lesysbot"`) over whatever the package declared, so `/d/lesysbot` is a constant the docs, the status screen and `share_dashboard` can all hardcode — and a fork can't move it by inheriting its parent's uid. `current_package()` resolves which package is live, preferring the lock so `render` and `lesysbot dashboard current` can never disagree.

Payload is `dashboard.json` (portable, what Grafana's export button gives you) or `dashboard.py` exposing `build(host, caps, ctx)` (host-adaptive). **An unavailable dashboard is not written** — and a previously rendered copy is *removed* when it stops being available, so a dashboard whose exporter went away doesn't keep serving empty panels. Deliberate asymmetry with tools, which stay visible with an explaining stub.

The bundled **System Overview** (`dashboards/system-overview/`) is the default, and is deliberately **basic**: Overview/CPU/Memory/Disk/Network only, which is what a stock `node_exporter`/`windows_exporter` always fills. Temperatures and GPU are omitted because they depend on hardware the machine may not have — there is no one-size-fits-all dashboard, and a blank panel is indistinguishable from a broken one. `basic_rows()` selects those rows out of `gen-dashboards.py`'s output; the **generator keeps its full sections**, because it still runs standalone for a stack with no LeSysBot and is what richer dashboard packages build from. The selection is always a *prefix* of the generator's panel list, which is why no `gridPos` reflow is needed — `tests/test_dashboards.py` asserts that prefix property so an interleaved hardware row fails loudly rather than leaving a vertical hole.

`setup/apply.seed_dashboards()` seeds the default into an **empty slot only** (`force=True` overrides), so re-running `lesysbot setup` never restores it over a dashboard the user chose; `reset_dashboard()` is the explicit way back, behind `lesysbot dashboard reset` and the panel's Reset button.
...
Seeding (`setup/apply.py`) installs bundled packages through the same rules and records them as `bundled: true`, so a fix to a bundled tool reaches machines that already have it.

### Control panel & CLI dispatch (`lesysbot/management/`, `__main__.py`)

A **loopback-only web control panel** for config + tools, **served by the service so it is always online** (`lesysbot run` → `serve_background()`; see the dispatch paragraph). This is the one listener in the project, so it is deliberately fenced: stdlib `http.server.ThreadingHTTPServer` bound to **127.0.0.1 only**, a **DNS-rebinding guard** (`_Handler._host_ok` rejects any non-loopback `Host` header → 403), **no auth** (trust boundary = a shell on the machine, same as editing `config.yaml`), and **zero new dependencies**. `management/page.py` inlines the single-page UI as a Python string so it ships with the package (no package-data wiring; survives a PyInstaller build); it follows the docs-site brand (slate + `brand-*` cyan tokens) and renders its header mark + favicon to SVG from `core/_logo` (the same sprite `core/banner.py` draws in the terminal — `gen_logo.py` emits both `MARK` (16px) and `MARK_32` into that module) so the web copy can't drift from the generated art. The page carries a 2-state light/dark toggle keyed on a `data-theme` attribute on `<html>` (a pre-paint script reads `localStorage['lesysbot-ui-theme']`, else the `prefers-color-scheme` media query drives it — no framework, no new dependency). API: `GET /api/status` (via `core/status.gather_status`), `/api/tools`, `/api/config`; `POST /api/config` (validates with `Settings(**yaml)` **before** writing), `/api/tools/{toggle,install,remove}`. **`/api/config` never carries a credential**: `management/secrets.py` masks every `token:`/`api_key:` value to `****<last four>` on the way out and `restore()`s the stored value on the way back in (a value still equal to the mask we sent means "keep it"; anything else is a new value typed by the user) — so an open tab, a screenshot or a saved page leaks nothing, and editing the model doesn't cost a retyped token. It works line-level (regex, not parse-and-redump) so comments/order/spacing survive and a config that *no longer parses* still masks; `${VAR}` refs and sub-`MIN_SECRET_LEN` placeholders (`api_key: ollama`) stay readable, sharing `core/redact.MIN_SECRET_LEN` so both surfaces judge "is this a secret" the same way. Two credentials sharing a mask raise `AmbiguousMask` → 400 rather than guessing which is which. Toggle calls `registry.set_enabled()` which persists to `mcp.state_file`, so a running bot applies it live via `Agent._watch_tool_state`; config edits need a restart. Registry mutations are serialized under one `threading.Lock`; the LLM health probe runs outside it.
...
`tests/` holds the pytest suite. Tests construct registries/agents over temp tool dirs and don't need a running LLM, messaging backend, or network. `asyncio_mode = "auto"` means async tests need no decorator; hermeticity comes from a monkeypatched `LESYSBOT_HOME`.

`test_macos_metrics.py`, `test_gen_dashboards.py` and `test_start_detect.py` are the exceptions to the tests-cover-`lesysbot/` rule: they exercise `dashboard/scripts/` by path, because `dashboard/` sits outside the package. All three run on **any** OS on purpose — the platform-specific logic is exactly what a developer on another platform would never otherwise execute. `test_macos_metrics` stubs its one OS entry point (`_run`); `test_gen_dashboards` pins which panels each host cut includes (plus a staleness check that the committed JSONs still match the generator); `test_start_detect` sources `start.sh` and drives `detect_capabilities_linux` against a fixture `/sys` tree, overriding `is_virtual`/`command` for the two facts that aren't in sysfs.

Installer tests share `tests/install_utils.py`: `make_github_zip()` builds GitHub-shaped zipballs (single `repo-ref/` root + commit SHA in the archive comment) and `FakeFetcher` serves them from a dict while recording requested URLs (used to assert the zipball candidate fallback order).
...
dir. `scripts/uninstall.{sh,ps1}` uninstall a clone-based (editable) install, which
the installer's own `--uninstall` cannot see.
...
adds `shellcheck --shell=sh --severity=warning` — an error-only pass cannot see a
bashism, since they are all SC3xxx *warnings*.
...
**Read the `.claude/skills/setup-wizard/` project skill before changing the
wizard, its prompts, unattended mode or the installers** — the module layout, the
`AutoUI` unattended seam, the navigation contract and the Grafana-password
fallback order live there, not here (and it is kept out of `lesysbot/` so
hatchling never bundles it into the wheel). Naming note: `lesysbot/setup/` is the
*install wizard*; `lesysbot/artifacts/` is the *package installer* — unrelated
modules.
...
Docs are written **user-first**: every page opens with the plain-language answer and the shortest path that works, and pushes internals into collapsed `<details><summary><b>…</b></summary>` blocks at the end (GitHub renders these natively; the site styles them). The rule when adding text: if a reader who just wants the thing to work doesn't need it, it belongs in a `<details>` block, not the main flow. Deliberately *not* numbered on the user-facing pages (`getting-started`, `usage`, `configuration`, `writing-tools`, `writing-dashboards`, `installing-tools`, `dashboards`, `service`, `management-ui`, `troubleshooting`) — headings are questions, and cross-links use slug anchors; the reference-ish pages (`adapters.md`, `architecture.md`, `building-windows-exe.md`, `CONTRIBUTING.md`) keep their numbering.
