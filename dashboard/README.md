...
The stack that records your machine's **CPU, memory, disk and network
(Ethernet & Wifi)** as time series and shows them on a ready-made Grafana
dashboard. Runs on **Linux, macOS and Windows**.

LeSysBot keeps **one** dashboard, and the default is deliberately basic —
temperatures and GPU depend on hardware you may not have. To change it, see
[Your dashboard](../docs/dashboards.md). This page is about the stack itself:
starting it, stopping it, and reconfiguring it.
...
**If you installed LeSysBot, you already have them** — `lesysbot setup` seeded
this whole folder into `~/.lesysbot/dashboard`, which is where the commands
below expect to run. Nothing to clone.

Want it standalone, without LeSysBot? The `dashboard/` folder is self-contained,
so copying just it is enough:
...
If LeSysBot is installed, this is the whole interface — one pair of commands on
every OS, run from anywhere:

```bash
lesysbot dashboard start    # bring the stack up
lesysbot dashboard stop     # take it down
lesysbot                    # print the real Grafana link (3001 if 3000 is taken)
```

They run the same per-OS scripts documented below, from
`~/.lesysbot/dashboard`, so you never have to know which one your machine
needs. The one exception is a Mac set up the Homebrew way — those services live
under `brew services`, so use `scripts/install-macos.sh` there.

To drive the scripts yourself, open a terminal in the `dashboard/` folder and
run the one command for your OS.
...
```bash
lesysbot dashboard stop          # every OS, except a Homebrew-path Mac
```
...
> Grafana provisions everything in `grafana/dashboards/generated/`, and nothing
> fills that directory for you. Run `lesysbot dashboard render`, or
> `python3 scripts/gen-dashboards.py --host <os> --have <caps> --out
> grafana/dashboards/generated/lesysbot.json`, or copy the portable JSON
> for your OS in there — otherwise Grafana comes up with no dashboards at all.
...
```bash
brew install macmon                  # Apple Silicon only; prebuilt binary
...
│   ├── dashboards/*.json         # committed portable cuts (see gen-dashboards.py)
│   └── dashboards/generated/     # what Grafana provisions, per host, git-ignored
...
hop) and Grafana provisioning with the datasource URL baked in (there's no
compose to interpolate `${PROM_URL}`), pointed at the same
`grafana/dashboards/generated/` the compose stacks mount. It's regenerated on
every run; machine-specific absolute paths are why it isn't committed.

Those dashboards are generated for the host rather than copied from
...
one is a usage error rather than a silently missing row. Every launch path writes
the result into `grafana/dashboards/generated/` — git-ignored, because those
files describe one machine.
